function [output_0, output_20, output_50, output_100] = Anisotropic_Diffusion_exp(img, t, k)
...
for iteration = 1:t
    for i=1:m
        for j=1:n
            ...
            c_N = exp(-(abs(del_N)/k)^2);
            c_S = exp(-(abs(del_S)/k)^2);
            c_W = exp(-(abs(del_W)/k)^2);
            c_E = exp(-(abs(del_E)/k)^2);
